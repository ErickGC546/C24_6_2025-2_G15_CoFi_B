...
setInterval(() => {
  ...
  console.log('[AI] 🧹 Caché de requests limpiado');
}, 10000);
...
export async function POST(req: Request) {
  try {
    ...
    // 🔑 Crear clave única ANTES de hacer cualquier operación
    const requestKey = `${userId}|${userQuestion}|${conversationId || 'none'}`;
    
    console.log(`[AI] 📨 Request recibido: ${requestKey.substring(0, 60)}...`);

    // 🔒 VERIFICAR SI YA HAY UNA SOLICITUD IDÉNTICA EN PROCESO
    if (processingRequests.has(requestKey)) {
      console.log(`[AI] ⚠️ DUPLICADO DETECTADO - Esperando resultado de la primera solicitud...`);
      const result = await processingRequests.get(requestKey);
      console.log(`[AI] ✅ Retornando resultado existente (ID: ${result.id})`);
    ...
    const processingPromise = (async () => {
      try {
        console.log(`[AI] 🔄 Procesando nueva solicitud...`);
...
Analiza los datos y proporciona una recomendación personalizada en formato JSON.`;
        ...
        // 💾 GUARDAR EN BD - SOLO SE EJECUTA UNA VEZ
        console.log(`[AI] 💾 Guardando en BD...`);
        ...
        console.log(`[AI] ✅ Guardado exitosamente: ${recommendation.id}`);
        ...
        console.error('[AI] ❌ Error en procesamiento:', error);
        throw error;
      }
    })();
    ...
    // Limpiar después de 3 segundos (permite manejar requests duplicados tardíos)
    setTimeout(() => {
      processingRequests.delete(requestKey);
      console.log('[AI] 🧹 Request limpiado del caché');
    }, 3000);
